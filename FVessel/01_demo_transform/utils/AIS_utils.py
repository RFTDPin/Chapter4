...
def data_filter(ais, camera_para):
    ...
    # lat = ais['lat']  # 假设字段名为lat
    # lon = ais['lon']  # 假设字段名为lon
    #
    # # 检查经纬度是否在合理范围内
    # if not (-90 <= lat <= 90) or not (-180 <= lon <= 180):
    #     # print(f"Warning: Invalid coordinates in AIS data - lat: {lat}, lon: {lon}")
    #     return False  # 跳过无效数据
    #
    # # 同样检查相机参数中的坐标
    # lat_cam, lon_cam = camera_para[0], camera_para[1]  # 根据实际参数结构调整
    # if not (-90 <= lat_cam <= 90) or not (-180 <= lon_cam <= 180):
    #     # print(f"Warning: Invalid camera coordinates - lat: {lat_cam}, lon: {lon_cam}")
    #     return False
    #
    # D_abs = count_distance((lat_cam, lon_cam), (lat, lon))
...
#             ais['x'], ais['y'] = x, y
#             AIS_visCurrent.loc[len(AIS_visCurrent)] = ais
#
#
#         # 情况2: 数据删除
#         elif flag == 'visTraj_del' or flag == 'ais_del':
#             AIS_vis = AIS_vis.drop(AIS_vis[AIS_vis['mmsi'] == ais['mmsi']].index)
#         AIS_vis = pd.concat([AIS_vis, AIS_visCurrent], ignore_index=True)
#
#     return AIS_vis

def preload_all_ais_data(ais_path):
    """
    在程序开始时，一次性加载所有AIS CSV文件到内存中的一个字典里。
    字典的key是时间戳字符串（例如 '2023_01_01_12_00_00'），value是对应的DataFrame。
    """
    ais_data_map = {}
    csv_files = sorted([os.path.join(ais_path, f) for f in os.listdir(ais_path) if f.endswith('.csv')])

    for file_path in tqdm(csv_files, desc="Loading AIS data"):
        # 从文件名中提取时间戳作为key
        timestamp_key = os.path.basename(file_path)[:-4]  # 移除.csv
        try:
            df = pd.read_csv(file_path, usecols=[1, 2, 3, 4, 5, 6, 7, 8], header=0)
            if not df.empty:
                ais_data_map[timestamp_key] = df
...
def transform_ais_for_frame(current_ais_df, camera_para, shape):
    """
    只处理当前帧的AIS数据，进行过滤和坐标转换。
    返回一个包含 (mmsi, x, y) 元组的列表。
    这个函数不再接收或修改历史轨迹数据。
    """
    transformed_points = []

    for _, ais_row in current_ais_df.iterrows():
        flag = data_filter(ais_row, camera_para)

        if flag == 'transform':
            x, y = visual_transform(ais_row['lon'], ais_row['lat'], camera_para, shape)
            if x is not None and y is not None:
                transformed_points.append((ais_row['mmsi'], x, y))
